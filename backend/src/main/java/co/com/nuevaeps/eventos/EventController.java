...
public class EventController {
 ...
 private final Set<String> allowed=Set.of("Jurídico","Financiero","Comercial","Técnico","Gestión Humana");
 ...
 @PostMapping("/events/{id}/registrations") public Registration register(@PathVariable Long id,@RequestBody RegisterRequest r){Event e=require(id);if(blank(r.provider())||r.people()==null||r.people().isEmpty()||r.people().size()>5)bad("Prestador y entre 1 y 5 asistentes requeridos");for(Person p:r.people()){if(!allowed.contains(p.role())||blank(p.firstName())||blank(p.lastName())||blank(p.phone())||blank(p.email())||!p.email().matches("^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$"))bad("Datos de asistente inválidos");}if(!e.slots().isEmpty()&&!e.slots().contains(r.slot()))bad("Selecciona una franja válida");Registration reg=new Registration(r.provider(),List.copyOf(r.people()),r.slot(),"CONFIRMADA");registrations.computeIfAbsent(id,k->new CopyOnWriteArrayList<>()).add(reg);System.out.println("[EMAIL MOCK] Confirmación enviada a "+r.people().stream().map(Person::email).toList());return reg;}
 ...
 @PostMapping("/events/{id}/commitments") public Commitment commitment(@PathVariable Long id,@RequestBody CommitmentRequest r){require(id);if(blank(r.provider())||!allowed.contains(r.role())||blank(r.text()))bad("Prestador, cargo y descripción requeridos");List<Commitment> list=commitments.computeIfAbsent(id,k->new CopyOnWriteArrayList<>());if(list.stream().filter(c->c.provider().equals(r.provider())).count()>=50)bad("Máximo 50 compromisos");Commitment c=new Commitment(commitmentIds.incrementAndGet(),r.provider(),r.role(),r.text(),false);list.add(c);return c;}
 @PostMapping("/events/{id}/commitments/finalize") public Map<String,String> finalizeCommitments(@PathVariable Long id,@RequestBody Map<String,String> request){require(id);return Map.of("status","FINALIZED_MOCK","provider",request.getOrDefault("provider",""));}
 @PostMapping("/events/{id}/acta") public Map<String,String> acta(@PathVariable Long id,@RequestParam String provider,@RequestParam MultipartFile file){require(id);if(blank(provider)||file.isEmpty()||file.getOriginalFilename()==null||!file.getOriginalFilename().toLowerCase().endsWith(".pdf"))bad("Adjunta un PDF válido");actas.put(id+":"+provider,file.getOriginalFilename());System.out.println("[EMAIL MOCK] Acta recibida para "+provider+"; pendiente firma del prestador");return Map.of("filename",file.getOriginalFilename(),"status","PENDING_PROVIDER_SIGNATURE");}
 ...
 @PostMapping("/mock/reminders") public Map<String,Object> reminders(){int count=registrations.values().stream().flatMap(Collection::stream).mapToInt(r->r.people().size()).sum();System.out.println("[EMAIL MOCK] Recordatorios diarios simulados para "+count+" inscritos");return Map.of("emailsSimulated",count);}
